...
library ButtonwoodLibrary {
    ...
    function getPools(
        ...
        address tokenA,
        address tokenB
    ) internal view returns (uint256 poolA, uint256 poolB) {
        (address token0, ) = sortTokens(tokenA, tokenB);
        (uint256 pool0, uint256 pool1, ) = IButtonwoodPair(pairFor(factory, tokenA, tokenB)).getPools();
    ...
    function getReservoirs(
        ...
        address tokenA,
        address tokenB
    ) internal view returns (uint256 reservoirA, uint256 reservoirB) {
        (address token0, ) = sortTokens(tokenA, tokenB);
        (uint256 reservoir0, uint256 reservoir1) = IButtonwoodPair(pairFor(factory, tokenA, tokenB)).getReservoirs();
