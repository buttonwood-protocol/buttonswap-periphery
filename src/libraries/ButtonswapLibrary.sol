...
//                            hex"9d6a75e3f6b514726acc925ddd0c37f1a015384e8fee40bd6fb982ad53db3761" // init code hash
                            initHashCode
                        )
                    )
                )
    ...
    {
        (address token0,) = sortTokens(tokenA, tokenB);
        (,,uint256 reservoir0, uint256 reservoir1,) = IButtonswapPair(pairFor(factory, tokenA, tokenB)).getLiquidityBalances();
    ...
    {
        (address token0,) = sortTokens(tokenA, tokenB);
        (uint256 pool0, uint256 pool1, uint256 reservoir0, uint256 reservoir1,) = IButtonswapPair(pairFor(factory, tokenA, tokenB)).getLiquidityBalances();
        (poolA, poolB, reservoirA, reservoirB) = tokenA == token0 ? (pool0, pool1, reservoir0, reservoir1) : (pool1, pool0, reservoir1, reservoir0);
    }
